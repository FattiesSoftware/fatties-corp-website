...
if (!class_exists('aacgfe_admin_notices')):
    ...
    final class aacgfe_admin_notices
    {
        ...
        public function aacgfe_add_message($notice)
        {
            ...
            add_action( 'admin_enqueue_scripts', array($this, 'aacgfe_load_script' ) );
    	...
    	public function aacgfe_load_script() {    	
            wp_register_style( 'aacgfe-review-css', AACGFE_URL . 'assets/css/aacgfe-admin-notice.css', null, null, 'all' );
             wp_register_script( 'aacfge-admin-notices-js', AACGFE_URL . 'admin/feedback/js/admin-review.js', array( 'jquery' ), AACGFE_VERSION, true );
    ...
    'ajax_callback' => $message['review']
                          ? 'aacgfe_admin_review_notice_dismiss'
                          : 'aacgfe_admin_notice_dismiss',
);


              $js_safe_id = str_replace('-', '_', $id);

            wp_localize_script('aacfge-admin-notices-js', 'AACGFENoticeData_' . $js_safe_id, $localized_data);
        }
        ...
        private function aacgfe_admin_notice_for_review( $id, $messageObj ){
            ...
            if(get_option( 'aacgfe-installDate' )){
                ...
                if (isset($diff_days) && $diff_days>= $days ) {
                    ...
                    wp_enqueue_script( 'aacfge-admin-notices-js' );
       ...
       function aacgfe_create_notice_content( $id, $messageObj ){
        ...
        $message="Thanks for using  <b>$plugin_name</b> - WordPress plugin. We hope you liked it ! <br> Please give us a quick rating, it works as a boost for us to keep working on more <a href='https://coolplugins.net' target='_blank'><strong>Cool Plugins</strong></a>!<br/>";
